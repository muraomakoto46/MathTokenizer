 ...
 **/
public class MathTokenizer4 {
	private String mathCode;
	public static void main(String[] args) throws IOException {
		// TODO 自動生成されたメソッド・スタブ
		String query="2.66";
		MathTokenizer4 mtz = new MathTokenizer4(query);
		System.out.println(mtz.separate());
	}

	public MathTokenizer4(String mathCode){
		...
		while(st.hasMoreTokens()){
			...
			if((result==null) && prey.matches("^(\\d+)([.]*)(\\d*)$")){
				...
				if(result==null){
					...
					result=(new MathTokenizer4(prey)).separate();
				}else{
					//右の枝分解。
					switch(tokenizer){
						case "+":
						...
						case "^":
							Double exp =(new MathTokenizer4(prey)).separate();
