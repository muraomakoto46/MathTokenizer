 ...
 **/
public class CopyOfMathTokenizer5 {
	private String mathCode;
	public static void main(String[] args) throws IOException {
		// TODO 自動生成されたメソッド・スタブ
		String query="2.66";
		CopyOfMathTokenizer5 mtz = new CopyOfMathTokenizer5(query);
		System.out.println(mtz.separate());
	}

	public CopyOfMathTokenizer5(String mathCode){
		...
		while(st.hasMoreTokens()){
			...
			if((result==null) && prey.matches("^(\\d+)([.]*)(\\d*)$")){
				...
				if(result==null){
					...
					result=(new CopyOfMathTokenizer5(prey)).separate();
				}else{
					//右の枝分解。
					switch(tokenizer){
						case "+":
						...
						case "^":
							result = Math.pow(result, (new CopyOfMathTokenizer5(prey)).separate());
							break;
						case "%":
							Double reminder=(new CopyOfMathTokenizer5(prey).separate());
